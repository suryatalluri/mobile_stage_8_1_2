...
Application.$controller('GooglemapsController', ['$scope', 'Utils', '$element', 'NgMap', '$timeout', '$http',
    function ($s, Utils, $el, NgMap, $timeout, $http) {
        'use strict';
        var prefabScope,
            _locations = [],
            _icon = '',
        ...
        function alterWayPoints(wayPointsObj) {
            if (wayPointsObj) {
                var newWayPoints = [];
        ...
        function propertyChangeHandler(key, newVal) {
            switch (key) {
                ...
                case 'destination':
                    ...
                    break;
                case 'waypoints':
                    $s.directionsData.wayPoints = newVal;
                    $s.$parent.widgetProps.stopover.show = newVal ? true : false;
                    break;
                case 'stopover':
                    alterWayPoints($s.waypoints);
                    break;
                case 'perimeter':
                    perimeter = newVal;
                    break;
